...
function PredCard({ m, now }) {
  ...
  useEffect(() => {
    ...
    if (!supabase) return;
    (async () => {
      const { data: v } = await supabase.from("votes").select("pick").eq("match_id", m.id);
      if (v) setVotes({ h: v.filter(x => x.pick === "h").length, d: v.filter(x => x.pick === "d").length, a: v.filter(x => x.pick === "a").length });
      const { count } = await supabase.from("score_predictions").select("*", { count: "exact", head: true }).eq("match_id", m.id);
      setPredCount(count || 0);
    })();
  }, [m.id]);

  const vote = async k => {
    if (myPick || busy || !supabase) return;
    setBusy(true);
    const { error } = await supabase.from("votes").insert({ match_id: m.id, pick: k });
    if (!error) {
  ...
  const lockScore = async () => {
    if (!form.name.trim() || form.h === "" || form.a === "" || busy || !supabase) return;
    setBusy(true);
    const { error } = await supabase.from("score_predictions").insert({ match_id: m.id, name: form.name.trim(), home: +form.h, away: +form.a });
    if (!error) { setSaved(true); lsSet("myname", form.name.trim()); setPredCount(c => c + 1); }
    setBusy(false);
  };
  ...
  return (
    <div className="mcard" style={{ padding: 18 }}>
      <div className="top">
        ...
        <span>{votes && total - (myPick ? 0 : 0) > 0 && (votes.h + votes.d + votes.a) > 0 ? `${votes.h + votes.d + votes.a} VOTES` : "BE FIRST TO VOTE"}</span>
      </div>
      ...
      {!supabase ? <p className="mono-dim" style={{ textAlign: "center" }}>Connect Supabase env vars to enable voting.</p>
        : !myPick ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            {opts.map(([k, l]) => (
              <button key={k} onClick={() => vote(k)} disabled={busy} className="lift"
                style={{ background: "var(--navy1)", border: "1px solid var(--line)", borderRadius: 10, padding: "13px 6px", fontFamily: "var(--disp)", fontWeight: 800, fontSize: 12, letterSpacing: ".05em", color: "var(--txt)", cursor: "pointer" }}>{l}</button>
            ))}
          </div>
        ) : (
      ...
      {supabase && (
        <div style={{ borderTop: "1px solid var(--line-soft)", paddingTop: 14 }}>
          {saved ? <div style={{ fontFamily: "var(--mono)", fontSize: 11.5, color: "var(--green)", textAlign: "center" }}>✓ Score locked in — good luck, {form.name}!</div> : <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 56px 14px 56px auto", gap: 8, alignItems: "center" }}>
              ...
              <button className="btn-g sm" onClick={lockScore} disabled={busy}>Lock in</button>
            </div>
...
function RecapCard({ m }) {
  ...
  useEffect(() => {
    if (!supabase) return;
    (async () => {
      const { data } = await supabase.from("score_predictions").select("name,home,away").eq("match_id", m.id);
  ...
  return (
    <div className="card">
      <div className="top" style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <span className="mono-dim">{m.group ? `GROUP ${m.group}` : ""} · FULL TIME</span><span className="badge ft">FT</span>
      </div>
...
/* ---------- tournament ballot ---------- */
const CATS = [
  ["winner", "🏆 World Cup winner", "team"], ["runnerup", "🥈 Runner-up", "team"],
  ["boot", "👟 Golden Boot", "text"], ["young", "🌟 Best Young Player", "text"],
  ["potm", "👑 Player of the Tournament", "text"], ["dark", "🐎 Dark Horse", "team"],
  ["exit", "💥 Surprise early exit", "team"], ["breakout", "🚀 Breakout star", "text"],
  ["fun", "🎪 Most entertaining team", "team"], ["shock", "⚡ Biggest tournament shock", "text"],
];
...
function Ballot() {
  ...
  useEffect(() => {
    ...
    if (!supabase) return;
    (async () => {
      const { data } = await supabase.from("ballots").select("name,picks").order("created_at");
      if (data) setAll(data);
    })();
  }, []);
  ...
  const submit = async () => {
    ...
    const { error } = await supabase.from("ballots").insert({ name: name.trim(), picks });
    if (!error) { setDone(true); lsSet("myname", name.trim()); setAll(a => [...(a || []), { name: name.trim(), picks }]); }
  };
  if (!supabase) return <p className="mono-dim">Connect Supabase env vars to enable the ballot.</p>;
  if (done) return (
    <div className="qp" style={{ maxWidth: "none", textAlign: "center" }}>
  ...
  return (
    <div className="card" style={{ padding: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14, flexWrap: "wrap", gap: 6 }}>
        <div style={{ fontFamily: "var(--disp)", fontWeight: 800, fontSize: 15, textTransform: "uppercase", letterSpacing: ".06em" }}>Tournament Ballot <span style={{ color: "var(--green)" }}>—</span></div>
  ...
  return (
    <main className="wrap" style={{ padding: "30px 20px 40px" }}>
      ...
      {tab === "up" && (loading ? <p className="mono-dim">Loading fixtures…</p> :
        <div className="grid auto-300">{up.map(m => <PredCard key={m.id} m={m} now={now} />)}</div>)}
      {tab === "ft" && (done.length
